...
class Command {
  ...
  constructor(busClient, pushSocket) {
    ...
    this.#xEtc = require('xcraft-core-etc')();
    if (this.#xEtc) {
  ...
  #onNetworkChange(err) {
    const hordeId = this._busClient?._busConfig?.hordeId;
    Command.abortAll(hordeId, err);
  }
  ...
  static abortAll(fromHordeId, err = null) {
    for (const [commandId, entry] of Command.#unsubList.entries()) {
      const {cmd, hordeId, unsub, callback} = entry;
      if (fromHordeId && hordeId !== fromHordeId) {
        continue;
      }
      ...
      if (callback) {
        callback(
          `The command ${cmd} for ${hordeId} is aborted${err ? `: ${err}` : ''}`
        );
      }
    }
  }
  ...
  #getToken() {
    ...
    return this.#xEtc ? xBus.getToken() : null;
  }
  ...
  send(cmd, data, which, finishHandler, options = {}, msgContext) {
    ...
    if (finishHandler) {
       ...
       */
      if (isRPC) {
        const routingKey = this.#getBus()
          .getCommander()
          .getRoutingKeyFromId(cmd, null, true);
