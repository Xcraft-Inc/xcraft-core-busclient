...
class Command {
  ...
  async _send(cmd, data, options) {
    ...
    return await this._busClient.command.sendAsync(
      cmd,
      data,
      orcName,
      options,
  ...
  /** @deprecated See sendAsync */
  send(cmd, data, finishHandler) {
    this._send(cmd, data)
      .then((res) => finishHandler(null, res))
      .catch((err) => finishHandler(err));
  }

  async sendAsync(cmd, data) {
    return await this._send(cmd, data);
  }

  /** @deprecated See nestedSendAsync */
  nestedSend(cmd, data, finishHandler) {
    this._send(cmd, data, {forceNested: true})
      .then((res) => finishHandler(null, res))
      .catch((err) => finishHandler(err));
  }

  /* Must be renamed (without Async) when everything is migrated */
  async nestedSendAsync(cmd, data) {
    return await this._send(cmd, data, {forceNested: true});
  }
}
