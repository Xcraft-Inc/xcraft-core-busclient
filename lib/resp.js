...
class Command {
  constructor(busClient, log, orcName, stack) {
    this._log = log;
    this._busClient = busClient;
    this._orcName = orcName;
    this._stack = stack || [];
...
class Resp {
  constructor(busClient, moduleName, orcName, stack) {
    this._busClient = busClient;

    this.log = require('xcraft-core-log')(moduleName, this);
    this.events = new Events(busClient, this.log, orcName);
    this.command = new Command(busClient, this.log, orcName, stack);
  }
