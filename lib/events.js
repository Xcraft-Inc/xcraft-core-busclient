 ...
 */
Events.prototype.subscribe = function(topic, handler, backend) {
  if (!topic.includes('::')) {
  ...
  const unsubscribe = handleReturn => {
    ...
    if (Object.getOwnPropertySymbols(this._handlers[topic]).length === 0) {
      this.unsubscribeAll(topic, backend);
    }
    return true;
  };
  ...
  this._handlers[topic][id] = handler;
  this._sub.subscribe(topic, backend);
 ...
 * @param {[string]} backend - Transport backend to use (or null for all).
 */
Events.prototype.unsubscribeAll = function(topic, backend) {
  if (!topic.includes('::')) {
    xLog.err(new Error().stack);
    throw new Error('namespace missing');
  }
  ...
  this._sub.unsubscribe(topic, backend);
  this._busClient.unregisterEvents(topic);
  delete this._handlers[topic];
};

Events.prototype.updateARP = function(orcName, token) {
  const {Router} = require('xcraft-core-transport');
  Router.updateARP(orcName, token);
};
 ...
 */
Events.prototype.send = function(topic, data, serialize, transports = []) {
  ...
  const which = topic.split('::')[0];

  var notifier = xBus.getNotifier();

  let busMessage = data;
  if (!data || !data._xcraftMessage) {
    busMessage = this._busClient.newMessage(topic, which, transports);
