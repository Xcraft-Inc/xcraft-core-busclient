...
Events.prototype.subscribe = function (topic, handler) {
  ...
  const escapeTopic = xUtils.regex.toAxonRegExpStr (topic);
  const eventsRegistry = this._busClient.getEventsRegistry ();

  eventsRegistry[escapeTopic] = msg => {
    /* FIXME: it's not safe. */
    if (msg.serialized) {
...
Events.prototype.unsubscribeAll = function (topic) {
  ...
  const escapeTopic = xUtils.regex.toAxonRegExpStr (topic);
  delete eventsRegistry[escapeTopic];
...
Events.prototype.send = function (topic, data, serialize) {
  ...
  var busMessage = this._busClient.newMessage (topic, which);
